...
// Values for the possible states of each intersection
    ...
    public static final Integer WHITE_GIPF = 2;
    public static final Integer BLACK = 3;
    public static final Integer WHITE = 4;
    ...
    public int playGame(GipfPlayable p1, GipfPlayable p2) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            piecesLeft[i % 2] -= 2;
            do {
                result = makeMove(players[i % 2].makeGipfMove(i % 2), (i % 2) + 1);
            } while (!result);
        ...
        while (gameIsOver() == 0) {
            piecesLeft[curPlayer]--;
            do {
                result = makeMove(players[curPlayer].makeGipfMove(curPlayer), (curPlayer + 3));
            } while (!result);
            curPlayer++;
            curPlayer %= 2;
    ...
    /**
     * This method checks for runs of four or more of the same color and 
     *   removes the pieces accordingly.
     */
    public void checkForFourInARow() {
        ...
        for (Run r : runs) {
            ...
            Integer pieceType = (r.playerValue + 1) % 2 + 3;
            Integer numPieces = 0;
        ...
        for (int col = 0; col < 9; col++) {
            for (int pos = 0; pos < board[col].length; pos++) {
                System.out.printf("%12s", Character.toString(letterValues[col])
                        + " " + Integer.toString(pos + 1) + ": " + board[col][pos]);
            }
            System.out.println();
        }
    }
